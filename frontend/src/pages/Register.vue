        ...
        <div class="register-form-container">
            <form id="registerForm" @submit="handleSubmit" novalidate autocomplete="off">
                ...
                <div class="form-group">
                    <label for="uName">Введите имя:
                    </label>
                    <input type="text" name="uName" placeholder="Введите имя сюда" id="uName" class="form-control"
                        v-model="registerObj.name" />
                    <p class="error-mess" v-if="errorObj.nameErr.length > 0">{{ errorObj.nameErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uEmail">Введите email:
                    </label>
                ...
                <div class="form-group">
                    <label for="uPass">Введите пароль:
                    </label>
                    <input type="password" name="uPass" placeholder="Введите пароль сюда" id="uPass"
                        class="form-control" v-model="registerObj.pass" autocomplete="on"/>
                    <p class="error-mess" v-if="errorObj.passErr.length > 0">{{ errorObj.passErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uPassConfirm">Повторите пароль:
                    </label>
                    <input type="password" name="uPassConfirm" placeholder="Введите повторный пароль сюда" id="uPassConfirm"
                        class="form-control" v-model="registerObj.confirm" autocomplete="on"/>
                    <p class="error-mess" v-if="errorObj.confirmErr.length > 0">{{ errorObj.confirmErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uPhone">Введите номер телефона:
                    </label>
                    <input type="tel" name="uPhone" placeholder="79" id="uPhone"
                        class="form-control" v-model="registerObj.phone" />
                    <p class="error-mess" v-if="errorObj.phoneErr.length > 0">{{ errorObj.phoneErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uBirth">Выберите день рождение:
                    </label>
                ...
                <div class="form-group">
                    ...
                    <p>Уже есть аккаунт? <router-link @click="scrollToTop()" to="/login">Войти</router-link>
                    </p>
                </div>
    ...
    methods: {
        ...
        checkForm: function () {
            ...
            if (!this.registerObj.name) {
                ...
                if (!/^[а-яА-ЯёЁa-zA-Z]+$/.test(this.registerObj.name.replace(/\s/g, ""))) {
            ...
            if (!this.registerObj.phone) {
                ...
                if (!this.registerObj.phone.startsWith('79')) {
                    this.errorObj.phoneErr.push('Phone numbers must start with 79');
                }
